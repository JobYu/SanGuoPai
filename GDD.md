...
*   **基礎傷害**：
    - 普通獲勝：`(玩家點數 - 敵將點數) × 10`
    - 完美 21 點 (Blackjack)：固定 `210` 基礎傷害並觸發特殊特效。
...
1.  **黃巾之亂**：基礎教學，过关所需$ 100-1000。
2.  **討伐董卓**：引入倍率概念，过关所需$ 1000-5000。
...
3. **招募武將 (Market)**：擊敗敵將後，進入招募介面，玩家可從隨機武將中選擇購買，或花費金錢擴展武將位上限。每名武將根據稀有度定價（Common: $100 / Uncommon: $250 / Rare: $600 / Legendary: $1500）。也可以選擇跳過招募直接進入下一關。
4. **武將槽位**：初始 5 名，顯示於畫面頂部。可透過購買擴展。

### 再戰與失敗
1. **失敗重啟**：玩家戰敗後可點擊返回標題畫面重新開始。
