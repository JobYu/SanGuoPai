...
class GameState {
    ...
    // Generic card renderer with options
    renderCard(card, options = {}) {
        const { isSelected = false, isMini = false, isBack = false, cardIndex = null, onclick = null, cornerOnly = false } = options;

        if (isBack) {
            const miniClass = isMini ? 'mini' : '';
            const onclickAttr = onclick ? ` onclick="${onclick}"` : '';
            return `<div class="playing-card back ${miniClass}"${onclickAttr}>?</div>`;
        }

        // Handle card as object (Card instance) or string (e.g., "A♠")
        let rank, suit;
        if (typeof card === 'string') {
            // Parse string like "A♠" or "10♥"
            suit = card.slice(-1);
            rank = card.slice(0, -1);
        } else {
            rank = card.rank;
        ...
        // For mini cards with cornerOnly, only show top-left corner
        if (isMini && cornerOnly) {
            return `
                <div class="playing-card corner-style ${miniClass} ${selectedClass}" style="color: ${color};"${onclickAttr}>
    ...
    // Legacy wrapper for backward compatibility (split hands)
    renderCornerCard(card, cardIndex, handIndex) {
        ...
        return this.renderCard(card, { isSelected, cardIndex });
    }
    ...
    renderBattleHTML() {
        ...
        return `
            ...
            (this.turn === 'PLAYER' && i === 1) ? this.renderCard(null, { isBack: true }) : this.renderCard(c)
        ).join('')}
                </div>
                ...
                <div class="hand-row player-hand">
                    ${this.currentHand().cards.map((c, i) =>
                        this.renderCard(c, { isSelected: this.selectedForDiscard.has(i), cardIndex: i, onclick: `game.toggleCardSelection(${i})` })
                    ).join('')}
                </div>
                `}
        ...
        const enemyHandHTML = `
            <div class="settlement-hand">
                ...
                <div class="hand-row mini">
                    ${this.enemyHand.cards.map(c => this.renderCard(c, { isMini: true, cornerOnly: true })).join('')}
                </div>
            </div>
        `;
            ...
            return `
                <div class="settlement-hand-item" style="border: 1px solid rgba(212,175,55,0.3); border-radius: 8px; padding: 15px; margin: 10px 0;">
                    ...
                    <div class="hand-row mini" style="margin-bottom: 10px;">
                        ${b.cards.map(c => this.renderCard(c, { isMini: true, cornerOnly: true })).join('')}
                    </div>
