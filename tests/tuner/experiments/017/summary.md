# Tuner Experiment 017

- **Timestamp**: 2026-04-22T10:56:28.303Z
- **Fitness**: 0.3426
- **Previous Best**: 0.4409
- **Status**: ❌ No improvement
- **Balance Score**: 0.4894
- **Skill Expression**: -0.15 stages
- **Elapsed**: 0.3s

## Parameter Changes

```json
{
  "enemy.money_target_multiplier": 0.95,
  "enemy.max_rounds_offset": 0,
  "enemy.ai_stand_threshold_base": 17,
  "enemy.ai_variance": 1,
  "difficulty.easy_multiplier": 0.75,
  "difficulty.hard_multiplier": 1.35,
  "difficulty.hard_ai_penalty": 0.4,
  "player.hands_per_battle": 4,
  "player.discards_per_battle": 4,
  "player.hit_limit": 3,
  "player.initial_generals": 3,
  "player.max_slots": 5,
  "skills.additive_base_multiplier": 1.3,
  "skills.multiplier_base_multiplier": 1.5,
  "rules.blackjack_payout": 2.5,
  "rules.double_down_allowed": true,
  "rules.split_allowed": true
}
```

## Per-Profile Results

### 新手
- **入門**: avg 3.98 stages (max 11)
- **標準**: avg 3.98 stages (max 7)
- **困難**: avg 3.06 stages (max 7)

### 休闲玩家
- **入門**: avg 3.76 stages (max 12)
- **標準**: avg 3.42 stages (max 9)
- **困難**: avg 3.28 stages (max 8)

### 硬核玩家
- **入門**: avg 3.54 stages (max 10)
- **標準**: avg 3.84 stages (max 9)
- **困難**: avg 3.18 stages (max 7)

## Targets vs Actual

| Profile | Difficulty | Target | Actual | Score |
|---------|------------|--------|--------|-------|
| 新手 | 入門 | 2 | 3.98 | 0.01 |
| 新手 | 標準 | 0.5 | 3.98 | 0.00 |
| 新手 | 困難 | 0 | 3.06 | 0.00 |
| 休闲玩家 | 入門 | 4 | 3.76 | 0.94 |
| 休闲玩家 | 標準 | 2.5 | 3.42 | 0.63 |
| 休闲玩家 | 困難 | 1 | 3.28 | 0.00 |
| 硬核玩家 | 入門 | 6 | 3.54 | 0.59 |
| 硬核玩家 | 標準 | 4 | 3.84 | 0.96 |
| 硬核玩家 | 困難 | 2.5 | 3.18 | 0.73 |

